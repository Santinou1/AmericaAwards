...
export const AuthProvider = ({ children }) => {
  ...
  const loadUser = async () => {
    try {
      ...
      const response = await axios.get('http://172.31.50.156:3000/api/auth');
  ...
  const login = async (email, usuarioPassword) => {
    try {
      setLoading(true);
      setError(null);
      console.log('Intentando login en:', 'http://172.31.50.156:3000/api/auth');
      
      const response = await axios.post('http://172.31.50.156:3000/api/auth', {
        email,
        usuarioPassword
      });
